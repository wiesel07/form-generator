...
public class GeneratorController {
	...
	ApiResult<PageResp<TableInfo>> list(PageReq<TableInfo> pageReq, TableInfoReq tableInfoReq) {
		...
		IPage page = new Page(pageReq.getPageNo(), pageReq.getPageSize());
	...
	public void batchCode(HttpServletRequest request, HttpServletResponse response, @PathVariable("tableNames") String tableNames) throws IOException {
	
		String[] tableNamesArr = new String[] {};
		tableNamesArr = JSON.parseArray(tableNames).toArray(tableNamesArr);
		byte[] data = generatorService.generatorCode(tableNamesArr, OWNER);
		response.reset();
		response.setHeader("Content-Disposition", "attachment; filename=\"wiesel.zip\"");
		response.addHeader("Content-Length", "" + data.length);
		response.setContentType("application/octet-stream; charset=UTF-8");
